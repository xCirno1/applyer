...
- \`inspect_application\` — open and retain a queued job's application in a
  ...
  types, current values, and option values. Decide each field's meaning from
  that context, but pass its
  field ID as the identifier. Inspection never changes a field or clicks a button.
- \`fill_application\` — after inspection, fill only the fieldId/value pairs
  ...
  button, advances the form, or submits it. The user reviews every answer.
