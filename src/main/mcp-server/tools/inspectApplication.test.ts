...
describe('inspectApplicationTool', () => {
  it('returns live questions and options to the agent', async () => {
    runInspectTask.mockResolvedValue({ status: 'inspected', jobId: 'job-1', mode: 'fill', fields: [{ fieldId: 'field-eligible', label: 'Eligible?', control: 'select', required: true, currentValue: '', options: [{ label: 'Yes', value: 'yes' }] }], buttons: [], storedDocuments: [], message: 'ready' })
