...
- `components/board/` — the job task board (Kanban columns, job cards, detail modal, captcha alert banner, filters, the pipeline overview sidebar). The captcha banner has two kinds of row: a job's fill paused on a challenge, and a job search a site refused headless that is now open in the application browser waiting for its challenge to clear (`main/browser/searchChallenge.ts`); both sit on the same gate, so Resume/Cancel work the same way, but a cancelled search only skips that site rather than failing anything.
...
- `components/navigation/` — cross-page chrome used by `App.tsx`'s `MainShell` itself rather than owned by any one screen (`IconRail`, the left rail switching between the Workspace, Job Discovery, Resume Variants and Runs screens, plus `DevBuildTag`, the top-bar dev-build marker). Same "split from `ui/` once there's a real consumer" reasoning as `workspace/` below, just one level higher — this is chrome *above* the screens, not glue *within* one screen.
...
- `components/runStats/` — the Runs screen: the control strip (`RunHeader`: which run is showing, Start/Stop, rename, delete), the statistic columns (`StatGroup`/`StatRow`, `SourceTable`, `ToolTable`), the paginated `RunTimeline`, and the plain-module halves (`runFormat.ts` for durations and run names, `describeRunEvent.ts` for the one sentence each timeline row gets). A run is a user-bounded stretch of agent work whose events the main process records (`src/main/runs/`) and folds into `RunStats`; the renderer only lays the numbers out, so a restart mid-run shows the same figures the database holds. The page that assembles them is `pages/RunStats/RunStatsPage.tsx`, state is `state/runsStore.ts`, and it is the fourth rail screen (`navigation/IconRail.tsx`) rather than a dock tab because the figures are too many for a strip a few rows tall; the dock still sits under it, so the terminal whose work the run measures stays in view.
...
| `RunHeader` | `runStats/RunHeader.tsx` |
| `StatGroup` / `StatRow` | `runStats/StatGroup.tsx` |
| `SourceTable` | `runStats/SourceTable.tsx` |
| `ToolTable` | `runStats/ToolTable.tsx` |
| `RunTimeline` | `runStats/RunTimeline.tsx` |
| `runFormat.ts` / `describeRunEvent.ts` | `runStats/runFormat.ts`, `runStats/describeRunEvent.ts` |
