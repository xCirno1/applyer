...
export default function IndexedJobRow({ item, compact = false }: { item: IndexedJobRecord; compact?: boolean }): ReactElement {
  ...
  if (compact) {
    return (
      ...
      >
        <Tag label={matched ? 'Matched' : 'Not selected'} tone={matched ? 'success' : 'neutral'} />
        ...
        <span className="shrink-0 text-[11px] text-text-faint">{formatRelativeTime(item.lastSeenAt)}</span>
        <a
          href={item.url}
          target="_blank"
          rel="noreferrer"
    ...
    >
      <div className="flex min-w-0 flex-1 flex-col gap-1">
        ...
        <div className="mt-0.5 flex items-center gap-1.5">
          ...
          <span className="text-[11px] text-text-faint">seen {formatRelativeTime(item.lastSeenAt)}</span>
        </div>
      </div>
